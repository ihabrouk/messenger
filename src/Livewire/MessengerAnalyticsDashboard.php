...
namespace App\Messenger\Livewire;
...
class MessengerAnalyticsDashboard extends Component
{
    ...
    public function getMetricsProperty()
    {
        ...
        return Cache::remember($cacheKey, 300, function () {
            ...
            $query = Message::where('created_at', '>=', $timeWindow);
            ...
            $failed = $query->where('status', MessageStatus::FAILED)->count();
            ...
            return [
                ...
                'avg_delivery_time' => $this->calculateAvgDeliveryTime($query),
                'avg_cost_per_message' => $total > 0 ? round($query->sum('cost') / $total, 4) : 0,
            ];
        });
    }
    ...
    public function getRecentActivityProperty()
    {
        ...
        return Cache::remember($cacheKey, 60, function () {
            $query = Message::with(['batch'])
                ->select(['id', 'recipient_phone', 'provider', 'channel', 'status', 'cost', 'created_at', 'batch_id'])
            ...
            return $query->get()->map(function ($message) {
                return [
                    'id' => $message->id,
                    'recipient' => substr($message->recipient_phone, 0, 3) . '****' . substr($message->recipient_phone, -3),
